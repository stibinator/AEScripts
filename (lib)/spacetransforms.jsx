...
function toWorldPos(fromLayer, fromPoint, t, posNull) {
    ...
    if (fromLayer) {
        ...
        if (posNull.name === "TWP-temp-delete") {
            posNull.remove();
        }
        return toWorld;
    } else {
...
function toWorldRotation(theLayer, t) {
    ...
    if (theLayer.threeDLayer) {
        theRotation = [
            theLayer.transform.xRotation.valueAtTime(t, false),
            theLayer.transform.yRotation.valueAtTime(t, false),
            theLayer.transform.zRotation.valueAtTime(t, false)
        ];
    } else {
        theRotation = theLayer.transform.rotation.valueAtTime(t, false);
...
function toWorldOrientation(theLayer, t) {
    ...
    t = defaultFor(t, app.project.activeItem.time);
    if (theLayer.threeDLayer) {
        theOrientation = theLayer.transform.Orientation.valueAtTime(t, false);
    } else {
        theOrientation = theLayer.transform.Orientation.valueAtTime(t, false);
    ...
    if (theLayer.parent === null) {
        ...
        return theOrientation + toWorldOrientation(theLayer.parent, t);
    }
}
...
function fromWorldScale(theLayer, worldScale, t) {
    t = defaultFor(t, app.project.activeItem.time);
    return 100 * [
        worldScale[0] / toWorldScale(theLayer, t)[0],
        worldScale[1] / toWorldScale(theLayer, t)[1]
    ];
}
