...
for (var i = 0; i < theLayers.length; i++){
    var theEffx = app.project.activeItem.layer(theLayers[i].index)("Effects");
    if (theEffx){
        for (var f= 1; f <= theEffx.numProperties; f++){
            if((! theEffx.property(f).enabled) && theEffx.property(f).isEffect){
...
alert("Removed " + removedCount + " effect" + ((removedCount != 1)?"s":""));
app.endUndoGroup();