...
(function (thisObj) {
    var scriptName = "knobs-n-buttons";
    ...
    function setHandleStyle(
        ...
        iconFill = iconFill | getLabelColour(targetLayer);
        var iconSizeArr =
    ...
    function makeNewHandleLayer(targetLayer) {
        var newShape = app.project.activeItem.layers.addShape();
        newShape.name = targetLayer.name + HANDLESUFFIX;
        //if the layer isn't a shape layer, create one and make it the parent of the layer
        makeHandleParentOfLayer(newShape, targetLayer, true);
        targetLayer.selected = false; //just to be sure
    ...
    function findLatestCommonAncestor(targetLayers) {
        for (var tl = 0; tl < targetLayers.length; tl++) {
            targetLayers[tl].lineage = findParent(targetLayers[tl], []);
        }
        if (targetLayers[0].lineage.length > 0) {
            var latestCommonAncestor = null;
            var foundCommonAncestor = false;
            for (
                var p = 0;
                p < targetLayers[0].lineage.length && !foundCommonAncestor;
                p++
            ) {
                latestCommonAncestor = targetLayers[0].lineage[p];
                tl = 1;
                var keepLooking = true;
                while (tl < targetLayers.length && keepLooking) {
                    if (
                        includes(
                            targetLayers[tl].lineage,
    ...
    function makeHandleParentOfLayer(newHandle, targetLayer, adoptTransforms) {
        newHandle.threeDLayer = targetLayer.threeDLayer;
        if (targetLayer.parent) {
            newHandle.parent = targetLayer.parent;
        }
        if (adoptTransforms) {
            newHandle.position.setValue(targetLayer.position.value);
            newHandle.rotation.setValue(targetLayer.rotation.value);
            newHandle.scale.setValue(targetLayer.scale.value);
        }
        targetLayer.parent = newHandle;
    }
    ...
    function getLabelColour(targetLayer) {
        if (!userLabelColours) {
            userLabelColours = readLabelColoursFromPrefs();
        }
        var label = targetLayer.label;
        return [
        ...
        function adjustIconShape(createNew) {
            ...
            if (app.project && app.project.activeItem) {
                ...
                for (var b = 0; b < selectedLyrsArr.length; b++) {
                    var targetLayer = selectedLyrsArr[b];
                    if (!(targetLayer instanceof ShapeLayer)) {
                        targetLayer = makeNewHandleLayer(targetLayer);
                    }
