...
(function () {
    function buildGUI(thisObj) {
        ...
        var numBarsEdit = numBarsGrp.add(
            "editText{alignment: ['right', 'center'], size: [25,20], justify: 'center'}"
        );
        ...
        var orientationDD = buttonsGrp.add("dropdownlist", undefined, [
            "Horizontal",
            "Vertical",
        ]);
        ...
        doItButton.onClick = function () {
            makeFrequencyBars(
                Math.round(numBarsSlider.value),
                orientationDD.value
            );
        };
        ...
        if (theComp instanceof CompItem) {
            ...
            for (var i = 0; i < theAudioLayers.length; i++) {
                ...
                if (theAudioLayer("Effects").canAddProperty("ADBE Aud HiLo")) {
                    var currentEffectCount =
                        theAudioLayer.property("Effects").numProperties;
                    var hiPass1 =
                        theAudioLayer("Effects").addProperty(
                            "ADBE Aud HiLo"
                        ).propertyIndex;
                    var loPass1 =
                        theAudioLayer("Effects").addProperty(
                    ...
                    for (var i = 0; i < numBars; i++) {
                        ...
                        kfLayer.name =
                            "levels-" +
                            Math.round(hiPassFreq) +
                            "-" +
                            Math.round(loPassFreq);
                        kfLayer.shy = true;
                        kfLayer.locked = true;
                        theComp.hideShyLayers = true;
                        var lColour =
                            (i / numBars) * lGradient[0] +
                            (1 - i / numBars) * lGradient[1];
                        var newBarL = theComp.layers.addSolid(
                            lColour,
                            kfLayer.name,
                            barWidth,
                            barHeight,
                        ...
                        newBarL.position.setValue([
                            (i + 1) * barWidth,
                            theComp.height / 2,
                        ]);
                        newBarL.parent = controllerNull;
                        newBarL.transform.anchorPoint.setValue([
                            barWidth / 2,
                            barHeight,
                        ]);
                        if (mono) {
                            newBarL.name =
                                ...
                                "-" +
                                Math.round(loPassFreq);
                            newBarL.scale.expression =
                                '[value[0], value[1] * thisComp.layer("' +
                                kfLayer.name +
                            ...
                            var rColour =
                                (i / numBars) * rGradient[0] +
                                (1 - i / numBars) * rGradient[1];
                            var newBarR = theComp.layers.addSolid(
                                rColour,
                                kfLayer.name,
                                barWidth,
                                barHeight,
                            ...
                            newBarR.scale.expression =
                                '[value[0], value[1] * thisComp.layer("' +
                                kfLayer.name +
                                '").effect("Right Channel")("Slider")];';
                            newBarR.transform.anchorPoint.setValue([
                                barWidth / 2,
                                0,
                            ]);
                        }
