...
function defineSteps(theLegs) {
    ...
    // and compute their position in a space orthoganal to the step vector
    // then store it.
    ...
    for (leg = 0; leg < theLegs.length; leg++) {
        stepData.firstKeyTime[leg] = theKFs[leg][0].time; //set the stepData.firstKeyTime[leg] to the first key on the first leg, it will get sorted out later
        stepData.lastKeyTime[leg] = 0; //is 0 correct?
        ...
        for (k = 0; k < theKFs[leg].length; k++) {
            theKeyTime = theLegs[leg].position.keyTime(theKFs[leg][k]);
            if (theKeyTime > stepData.lastKeyTime[leg]){
                stepData.lastKeyTime[leg] = theKeyTime;
            } else if (theKeyTime < stepData.firstKeyTime[leg]) {
                stepData.firstKeyTime[leg] = theKeyTime;
            }
            // now rotate and scale the keyframes and their handles.
            // After effects has no keyframe object so...

            stepData.legKFs[leg][k] = {
                //kTime will be normalised next
                kTime: theKeyTime,
        ...
        forwardStepTime = stepData.lastKeyTime[leg] - stepData.firstKeyTime[leg];
        // calculate the time of each KF, normalised to the total length of time spent moving the leg forward
        // multiply this by the distance moved forward to work out how far along each walk cycle the keyframe should go
        for (var k = 0; k < stepData.legKFs[leg].length; k++) {
            // 0 <= kTime <= 1. kTime defines the proportional positiion in the step cycle for each KF
            stepData.legKFs[leg][k].kTime = (stepData.legKFs[leg][k].kTime - stepData.firstKeyTime[leg]) / forwardStepTime;
        }
    }
    // } catch (e) {
...
function walkinate(theBody, theStepData, stepOverlap) {
    ...
    var legOffset = []; // vector - the position of each leg in relation to the body at kf[0]]
    var numLegs = stepData.legLayers.length;
    //step overlap must be defined - UI for this, or use original KFs?
    var totalStepLength = stepData.stepMag * (1 - stepOverlap / 100) / thelegs.length; // scalar - the length of a total step Cycle
    // calculate the rest position for each leg
